...
import random
from serve.utils_general import combine_dataset_image
import wandb
from serve.utils_clip import get_embeddings
from serve.utils_llm import get_llm_output
from serve.utils_vlm import get_vlm_output

# from modify.utils_llava import interact_with_llava
...
# def classify(similarity_A_C, similarity_B_C, threshold=0.3, confidence=0.95):
#     """
#     判斷兩組數據中符合概念 C 的比例差異，並計算信賴區間（CI）
    
#     :param similarity_A_C: Group A 的 cosine similarity 數據
#     :param similarity_B_C: Group B 的 cosine similarity 數據
#     :param threshold: 判斷是否包含概念 C 的閾值（預設 0.3）
#     :param confidence: 信賴區間（預設 95%）
    
#     :return: 差異 (P(hyp in A) - P(hyp in B)) 及 95% CI
#     """
#     similarity_A_C = np.array(similarity_A_C, dtype=np.float64)
#     similarity_B_C = np.array(similarity_B_C, dtype=np.float64)
    
#     # 避免數據為空時發生錯誤
#     if similarity_A_C.size == 0 or similarity_B_C.size == 0:
#         return 0.0, (0.0, 0.0)
    
#     # 計算 P(hyp in A) 和 P(hyp in B)
#     percent_correct_a = np.mean(similarity_A_C > threshold)
#     percent_correct_b = np.mean(similarity_B_C > threshold)
#     diff = percent_correct_a - percent_correct_b
    
#     # 準備數據與標籤
#     data = np.concatenate([similarity_A_C, similarity_B_C]).flatten()
#     labels = np.concatenate([np.ones(len(similarity_A_C)), np.zeros(len(similarity_B_C))])
    
#     # 確保數據長度匹配
#     assert data.shape[0] == labels.shape[0], "data 和 labels 長度不匹配"
    
#     def stat_func(sample_data):
#         a_mask = labels == 1
#         b_mask = labels == 0
        
#         if np.sum(a_mask) == 0 or np.sum(b_mask) == 0:
#             return 0.0  # 避免無數據時發生錯誤
        
#         percent_correct_a = np.mean(sample_data[a_mask] > threshold)
#         percent_correct_b = np.mean(sample_data[b_mask] > threshold)
        
#         return percent_correct_a - percent_correct_b
    
#     # 進行 bootstrap 計算信賴區間
#     res = bootstrap((data,), stat_func, confidence_level=confidence, random_state=42, n_resamples=1000, method='percentile')
...
#         self, hypotheses: List[str], dataset1: List[dict], dataset2: List[dict], metrics_path: str
#     ) -> List[dict]:
...
#             # 寫入表頭
#             if not file_exists:
#                 writer.writerow(scored_hypotheses[0].keys())
#             # 寫入數據
#             for metrics in scored_hypotheses:
...
#         c_d, ci = classify(scores1, scores2, threshold=self.args["classify_threshold"])
#         metrics["correct_delta"] = c_d
...
class VLMRanker(Ranker):
    ...
    # # simple version: 100 pictures for one time
    # def score_hypothesis(self, hypothesis: str, dataset: List[dict]) -> List[float]:
    #     scores = []
    #     invalid_scores = []
    ...
    # random sample 25 pictures for once time
    def score_hypothesis(self, hypothesis: str, dataset: List[dict]) -> List[float]:
        scores = []
        invalid_scores = []
        random.seed(self.args["seed"])
        dataset_25 = random.sample(dataset, 25)
            
        for i in trange(0, len(dataset_25)):
            item = dataset_25[i]
            prompt = f"Does this image contain {hypothesis.replace('and ', '')}? Please just answer yes or no."  # TODO: why this prompt
            output = get_vlm_output(item["path"], prompt, self.args["model"])
            if "yes" in output.lower():
                scores.append(1)
            elif "no" in output.lower():
                ...
                invalid_scores.append(output)
        # print(f"Percent Invalid {len(invalid_scores) / len(dataset_25)}")
        # print(f"scores: {scores}")
        return scores
    
    # # combine 100 pictures to 5 big pictures for once time
    # def score_hypothesis(self, hypothesis: str, dataset: List[dict]) -> List[float]:
    #     scores = []
    #     invalid_scores = []
    #     random.seed(self.args["seed"])
    ...
    #     prompt = f'''
    #         This image contains one group of images. 10 images are shown in the top row, and 10 images are shown in the bottom row.
    #         I am a machine learning researcher trying to figure out whether each image matches {hypothesis.replace('and ', '')}. For each image that matches the hypothesis, assign 1 point. The total score will range from 0 to 20.
    #         Your response should only be a single number within the range of 0 to 20, representing the total score.
    #         '''
    #     image_path = f"cache/images/ranked.png"
    #     for i in range(5):
    #         each_scores = [0] * 20
    #         datas = dataset[20*i:20*(i+1)]
    #         combine_dataset_image(datas, image_path)
    #         output = get_vlm_output(image_path, prompt, self.args["model"])
    #         match = re.search(r'\d+', output)
    #         if match:
    #             each_scores[:int(match.group())] = [1] * int(match.group())
    #         scores.extend(each_scores)
    #     #     if match:
    #     #         scores.append(float(match.group()))
    #     # print(f"scores: {scores}")
    #     return scores

# # VLM without api call
# class VLMRanker(Ranker):
#     def __init__(self, args: Dict):
#         super().__init__(args)
...
#     #         item = dataset[i]
#     #         prompt = f"Does this image contain {hypothesis.replace('and ', '')}?"  # TODO: why this prompt
#     #         output = interact_with_llava(item["path"], prompt, self.args["model"])
#     #         if "yes" in output.lower():
#     #             scores.append(1)
#     #         elif "no" in output.lower():
#     #             scores.append(0)
...
#     #         datas = dataset[20*i:20*(i+1)]
#     #         combine_dataset_image(datas, image_path)
#     #         output = interact_with_llava(image_path, prompt, self.args["model"])
#     #         match = re.search(r'\d+', output)
#     #         if match:
#     #             each_scores[:int(match.group())] = [1] * int(match.group())
#     #         scores.extend(each_scores)
