...
# modify saving image to folder + loading saved image
class Proposer:
    def __init__(self, args: Dict):
        self.args = args

    def propose(
        self, dataset1: List[Dict], dataset2: List[Dict], image_path: str, output_path: str, modify: bool = False
        ...
        for i in range(self.args["num_rounds"]):
            ...
            image_path = f"{image_path}_{i}"
            hypotheses, logs = self.get_hypotheses(sampled_dataset1, sampled_dataset2, image_path, output_path, modify)
...
class LLMProposer(Proposer):
    ...
    def get_hypotheses(
        self, sampled_dataset1: List[Dict], sampled_dataset2: List[Dict], image_path, output_path, modify
    ) -> Tuple[List[str], Dict]:
...
# save image to folder + load saved image
class VLMProposer(Proposer):
    ...
    def get_hypotheses(
        self, sampled_dataset1: List[Dict], sampled_dataset2: List[Dict], image_path: str, output_path: str, modify: bool
    ) -> Tuple[List[str], Dict]:
        ...
        if not os.path.exists(image_path):
            print("saving image!")
            save_data_diff_image(sampled_dataset1, sampled_dataset2, image_path, modify)
        ...
        # pattern = r'\* "(.*?)"'  # 匹配所有 * "..." 格式的句子 ver1
        # hypotheses = re.findall(pattern, output)
        
        # if hypotheses == []:
        pattern = r'\. "(.*?)"' # 匹配所有 . "..." 格式的句子 ver2
        hypotheses = re.findall(pattern, output)
        # if hypotheses == []:
        #     pattern = r'\* (.*?)' # 匹配所有 * ... 格式的句子 ver3
        #     hypotheses = re.findall(pattern, output)
