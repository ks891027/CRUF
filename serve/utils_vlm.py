...
from serve.global_vars import BLIP_FEATURE_URL, BLIP_URL, LLAVA_URL, VLM_CACHE_FILE, LLAVA_NEXT_URL, QWEN2_URL, QWEN5_URL
...
def get_vlm_output(image: str, prompt: str, model: str) -> str:
    ...
    if model in ["blip", "llava", 'llava_next', "qwen2", "qwen2.5"]:
        files = {"image": open(image, "rb").read()}
        text_data = {"text": prompt}
        url = {
            "blip": BLIP_URL,
            "llava": LLAVA_URL,
            "llava_next": LLAVA_NEXT_URL,
            "qwen2": QWEN2_URL,
            "qwen2.5": QWEN5_URL,
        }[model]
    ...
    elif model == "gpt-4o-mini":  # Add GPT-4V support
